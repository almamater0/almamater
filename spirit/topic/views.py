...
@login_required #requiere login y el decorator me lleva a accounts/login que es de django eb vez de user/ lo que sea de spirit. hay que cambiar el decorator
...
def index_active(request):
    ...
    topics = yt_paginate(
        topics,
        per_page=8,#config.topics_per_page,
        page_number=request.GET.get('page', 1)
    )
